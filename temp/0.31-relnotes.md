...
[Multihost](https://gohugo.io/content-management/multilingual/#configure-multilingual-multihost) means that you can have a `baseURL` per language, for example `https://no.example.com` and `https://en.example.com`. This is seamlessly integrated into Hugo and the built-in web server with live reload and `--navigateToChanged` etc. just works. A related enhancement in this release is the support for as many static dirs as you need, with intelligent language overrides, forming a big union file system.
...
* For mapping of translated content, Hugo now consider the full path of the content file, which makes it possible with translation of duplicate content filenames such as `index.md`. A specific translation key can be specified with the new `translationKey` front matter variable. See [#2699](https://github.com/gohugoio/hugo/issues/2699).
